...
def temporal_loss(out1, out2, w, labels, device):
    ...
    def masked_crossentropy(out, labels):
        ...
        if nbsup > 0:
            ...
            return loss, nbsup
        return torch.FloatTensor([0.]).to(device), 0
...
def train(model, seed, k=100, alpha=0.6, lr=0.002, beta2=0.99, num_epochs=150,
    ...
    Z = torch.zeros(ntrain, n_classes).float().to(device)        # intermediate values
    z = torch.zeros(ntrain, n_classes).float().to(device)        # temporal outputs
    outputs = torch.zeros(ntrain, n_classes).float().to(device)  # current outputs
    ...
    for epoch in range(num_epochs):
        ...
        for i, (images, labels) in enumerate(train_loader):  
            ...
            loss, suploss, unsuploss, nbsup = temporal_loss(out, zcomp, w, labels, device)
    ...
    # test
    model.eval()
    acc = calc_metrics(model, test_loader, device)
    if print_res:
    ...
    model.eval()
    acc_best = calc_metrics(model, test_loader, device)
    if print_res:
